...
`nativeButton={false}` follows the *material*. A glass `Button` is four stacked
layers, which is invalid inside a native `<button>`, so it is not one — and Base
UI warns unless the composing component is told so. The solid `accent` close is
a real button again and says nothing. See
