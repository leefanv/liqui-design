...
`nativeButton={false}` on the trigger: a *glass* `Button` is not a native
`<button>` — it cannot be, the glass anatomy is invalid inside one — and Base UI
warns unless the composing component is told so. A solid `accent` or `danger`
button is one, and must not be given it. See
[Button](/docs/components/button#only-the-glass-variant-opts-out-of-button).
