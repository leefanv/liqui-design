---
title: Theming
description: Fourteen custom properties, four global optics defaults, and four multipliers — plus the editor that writes them for you.
---
...
property. Change fourteen variables and the whole library changes with them —
...
| `--lq-control` | The resting fill of a *flat* control — checkbox, radio, toggle. Not glass: nothing blurs behind it. |
| `--lq-control-rim` | That control's hairline edge. Reads dark on a light theme and light on a dark one. |
| `--lq-accent` | Prominent buttons, focus rings, checked state, selection. |
...
If you installed through the shadcn CLI, all fourteen are already in your
...
fourteen tokens and are written into a `<style>` at `:root`. That is for a theme
